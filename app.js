const STORAGE_KEY='asset-manager-v4-0';
const OLD_KEYS=['asset-manager-v3-9-2','asset-manager-v3-9','asset-manager-v3-8-1','asset-manager-v3-8','asset-manager-v3-6','asset-manager-v3-7','asset-manager-v3-6','asset-manager-v3-9','asset-manager-v3-8-1','asset-manager-v3-8','asset-manager-v3-7','asset-manager-v3-6','asset-manager-v3-5','asset-manager-v3-0','asset-manager-v2-3','asset-manager-v2-2','asset-manager-v2-1','asset-manager-v2-0','asset-manager-v1-5','asset-manager-v1-4','asset-manager-v1-3','asset-manager-v1-2','asset-manager-v1-1'];
...
function loadState(){let saved=localStorage.getItem(STORAGE_KEY);if(!saved){for(const k of OLD_KEYS){if(localStorage.getItem(k)){saved=localStorage.getItem(k);break;}}} if(saved){try{const s=JSON.parse(saved);return {...{assets:[],debts:[],snapshots:[],exchanges:[]},...s,version:'4.0'};}catch{}} return {version:'4.0',assets:[],debts:[],snapshots:[],exchanges:[],updatedAt:new Date().toISOString()};}
function save(){state.version='4.0';state.updatedAt=new Date().toISOString();localStorage.setItem(STORAGE_KEY,JSON.stringify(state));render();scheduleAutoGithubBackup();}
...
function isCardDebt(d){return String(d.type||'')==='카드 결제예정액';}
function cardDebtTotal(){return state.debts.filter(isCardDebt).reduce((s,d)=>s+debtAmount(d),0);}
function cardDueLabel(){const rows=state.debts.filter(isCardDebt);if(!rows.length)return '예정 없음';const today=new Date();today.setHours(0,0,0,0);const future=rows.filter(d=>d.dueDate).sort((a,b)=>String(a.dueDate).localeCompare(String(b.dueDate)));if(!future.length)return rows.length+'건 등록';const next=new Date(future[0].dueDate+'T00:00:00');const diff=Math.round((next-today)/86400000);return diff>=0?`${diff}일 후 결제`:`${Math.abs(diff)}일 지남`;}
function formatDueDate(d){return d.dueDate?` · 예정일 ${esc(d.dueDate)}`:'';}
...
function render(){
 refreshFxBoard();
 $('netWorth').textContent=money(netWorth());$('totalAssets').textContent=money(totalAssets());$('totalLiabilities').textContent=money(totalDebts());if($('cardDueTotal'))$('cardDueTotal').textContent=money(cardDebtTotal());if($('cardDueSoon'))$('cardDueSoon').textContent=cardDueLabel();if($('totalInvestCost'))$('totalInvestCost').textContent=money(totalInvestCost());if($('totalProfit')){$('totalProfit').textContent=signedMoney(totalProfit());$('totalProfit').className=profitClass(totalProfit());}if($('totalProfitPct')){$('totalProfitPct').textContent=signedPct(totalProfitPct());$('totalProfitPct').className=profitClass(totalProfit());}
...
function renderDebts(){const cardEl=$('cardDebtSummary');if(cardEl)cardEl.innerHTML=`카드 결제예정액: <b>${money(cardDebtTotal())}</b> <span>${esc(cardDueLabel())}</span>`;$('debtList').innerHTML=state.debts.length?state.debts.map(d=>`<div class="item"><div><div class="name">${esc(d.name)}</div><div class="meta">${esc(d.type)} · ${esc(d.currency||'KRW')} ${d.rate?`· 금리 ${d.rate}%`:''}${formatDueDate(d)}</div></div><div class="meta">${isCardDebt(d)?'카드 결제예정액':'부채 잔액'}</div><div class="amount">${money(debtAmount(d))}</div><button onclick="editDebt('${d.id}')">수정</button><button class="danger" onclick="removeDebt('${d.id}')">삭제</button></div>`).join(''):'<p class="note">등록된 부채가 없습니다.</p>';}
...
function resetDebtForm(){editingDebtId=null;$('debtSubmitBtn').textContent='저장';$('debtCancelBtn').classList.add('hidden');$('debtForm').reset();$('debtCurrency').value='KRW';if($('debtDueDate'))$('debtDueDate').value='';}
...
async function doGithubBackup(auto=false){
 try{
  ...
  const payload={state,prefs,backupAt:new Date().toISOString(),backupMode:auto?'auto':'manual'};
  ...
  if(auto)updateAutoBackupStatus(msg);else setStatus(msg);
 }catch(e){const msg=(auto?'자동 백업 실패: ':'백업 실패: ')+e.message;if(auto)updateAutoBackupStatus(msg);else setStatus(msg)}
...
if('serviceWorker'in navigator)navigator.serviceWorker.register('./sw.js?v=40').catch(()=>{});updateAssetPreview();render();autoUpdateFx().then(()=>refreshCryptoPrices(false)).catch(()=>refreshCryptoPrices(false));
